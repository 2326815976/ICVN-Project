...
function getNodeTitle(text: string, fallback: string) {
...
function mapShapeKindToBackendType(kind: ShapeNodeKind | undefined, nodeType: AppNode["type"]) {
  if (nodeType === "lineNode") {
    return "lineNode";
  }
...
function buildTaskResultNode(node: AppNode, index: number, graphId: string, nowIso: string): TaskGraphNode {
  const position =
  ...
  if (node.type === "lineNode") {
    ...
    const taskNode = {
      ...
      properties: toJsonRecord({
        ...
        anchorStart: node.data.anchorStart,
        anchorEnd: node.data.anchorEnd,
      }),
      position,
      createdAt: nowIso,
      updatedAt: nowIso,
    } as TaskGraphNode & Record<string, unknown>;

    taskNode.data = { ...node.data };
    if (typeof node.width === "number") taskNode.width = node.width;
    if (typeof node.height === "number") taskNode.height = node.height;
    if (typeof node.initialWidth === "number") taskNode.initialWidth = node.initialWidth;
    if (typeof node.initialHeight === "number") taskNode.initialHeight = node.initialHeight;
  ...
  const taskNode = {
    ...
    graphId,
    type: mapShapeKindToBackendType(node.data.kind, node.type),
    label,
    properties: toJsonRecord({
      kind: node.data.kind,
      text: node.data.text,
...
function buildTaskResultEdge(edge: AppEdge, index: number, graphId: string, nowIso: string): TaskGraphEdge {
  ...
  const taskEdge = {
    ...
    properties: toJsonRecord({
      ...
      pathStyle: edgeData.pathStyle,
      dashed: edgeData.dashed,
      marker: edgeData.marker,
      color: edgeData.color,
      labelOffset: edgeData.labelOffset,
      labelAnchorPosition: edgeData.labelAnchorPosition,
      labelPlacementMode: edgeData.labelPlacementMode,
      manualRoute: edgeData.manualRoute,
      manualRouteMode: edgeData.manualRouteMode,
      isEventEdge: edgeData.isEventEdge,
      eventID: edgeData.eventID,
      eventOverview: edgeData.eventOverview,
      eventDescription: edgeData.eventDescription,
      name1: edgeData.eventName1,
      name2: edgeData.eventName2,
    }),
    createdAt: nowIso,
    updatedAt: nowIso,
  } as TaskGraphEdge & Record<string, unknown>;

  taskEdge.source = edge.source;
  taskEdge.target = edge.target;
  if (edge.sourceHandle) taskEdge.sourceHandle = edge.sourceHandle;
  if (edge.targetHandle) taskEdge.targetHandle = edge.targetHandle;
  taskEdge.data = { ...edgeData };
  return taskEdge as TaskGraphEdge;
}

export function buildTaskResultFromGraphDocument(
  ...
  return {
    meta: {
      sourceType: options?.sourceType?.trim() || "import",
    },
    nodes: document.nodes.map((node, index) => buildTaskResultNode(node, index, graphId, nowIso)),
    edges: document.edges.map((edge, index) => buildTaskResultEdge(edge, index, graphId, nowIso)),
    events: [],
  };
}
